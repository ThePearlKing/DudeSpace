...
func build(b, dir: Vector3) -> void:
	...
	# ---- atrium ----
	_plate(Vector3(12.6, 0.5, 12.6),
		Transform3D(abas, _C + _u0 * (_rF - 0.25)), Vector3.ZERO, DARK, 0.0)
	...
	_plate(Vector3(3.7, 0.5, 5.2), acxf, Vector3(-4.45, 0, 0), DARK, 0.0)
	var awxf := Transform3D(abas, _C + _u0 * (_rF + 3.0))
	_plate(Vector3(12.6, 6.5, 0.5), awxf, Vector3(0, 0, -6.05), STEEL, 0.0)
	_plate(Vector3(1.3, 6.5, 0.5), awxf, Vector3(5.65, 0, 6.05), STEEL, 0.0)
	_plate(Vector3(1.3, 6.5, 0.5), awxf, Vector3(-5.65, 0, 6.05), STEEL, 0.0)
	_plate(Vector3(0.5, 6.5, 12.6), awxf, Vector3(6.05, 0, 0), STEEL, 0.0)
	# -X wall: 2.4m doorway to the bunk nook
	_plate(Vector3(0.5, 6.5, 4.9), awxf, Vector3(-6.05, 0, 3.65), STEEL, 0.0)
	_plate(Vector3(0.5, 6.5, 4.9), awxf, Vector3(-6.05, 0, -3.65), STEEL, 0.0)
	_plate(Vector3(0.5, 1.9, 2.4), awxf, Vector3(-6.05, 2.3, 0), STEEL, 0.0)
	...
	_sign("[ SURFACE GATE ]", abas, _C + _u0 * (_rF + 3.6),
		Vector3(4.5, 0, -5.5), 0.0)
	...
	# ---- bunk nook (-X of atrium): the ONE sleeping spot ----
	var nkc := _C + _u0 * (_rF - 0.25)
	_plate(Vector3(8.6, 0.5, 8.6), Transform3D(abas, nkc),
		Vector3(-10.5, 0, 0), DARK, 0.0)
	var nwxf := Transform3D(abas, _C + _u0 * (_rF + 2.0))
	_plate(Vector3(0.5, 4.5, 8.6), nwxf, Vector3(-14.7, 0, 0), STEEL, 0.0)
	_plate(Vector3(8.6, 4.5, 0.5), nwxf, Vector3(-10.5, 0, 4.3), STEEL, 0.0)
	_plate(Vector3(8.6, 4.5, 0.5), nwxf, Vector3(-10.5, 0, -4.3), STEEL, 0.0)
	_plate(Vector3(0.5, 4.5, 3.1), nwxf, Vector3(-6.3, 0, 2.75), STEEL, 0.0)
	_plate(Vector3(0.5, 4.5, 3.1), nwxf, Vector3(-6.3, 0, -2.75), STEEL, 0.0)
	_plate(Vector3(0.5, 1.2, 2.4), nwxf, Vector3(-6.3, 1.65, 0), STEEL, 0.0)
	_plate(Vector3(8.6, 0.5, 8.6), Transform3D(abas, _C + _u0 * (_rF + 4.5)),
		Vector3(-10.5, 0, 0), DARK, 0.0)
	for bz in [-1.9, 1.9]:
		...
		gring.mesh = grm
		gring.material_override = Destructible.make_material(AMBER, 1.8)
		add_child(gring)
		gring.global_transform = Transform3D(abas, _C + _u0 * (_rF + 0.14))
		gring.translate_object_local(Vector3(-12.2, 0, bz))
		var bunk := MeshInstance3D.new()
		bunk.mesh = IcosaColony._cham_mesh(2.6, 0.28, 1.4, 0.3)
		bunk.material_override = Destructible.make_material(STEEL, 0.25)
		add_child(bunk)
		bunk.global_transform = Transform3D(abas, _C + _u0 * (_rF + 0.7))
		bunk.translate_object_local(Vector3(-12.2, 0, bz))
	var nl := MeshInstance3D.new()
	var nlm := CylinderMesh.new()
	nlm.top_radius = 0.7
	nlm.bottom_radius = 0.7
	nlm.height = 0.08
	nl.mesh = nlm
	nl.material_override = Destructible.make_material(Color("#f2ead8"), 1.8)
	add_child(nl)
	nl.global_transform = Transform3D(abas, _C + _u0 * (_rF + 4.2))
	nl.translate_object_local(Vector3(-10.5, 0, 0))
	...
	for i in SEGS:
		...
		var wxf := Transform3D(fb, _C + up * (_rF + 2.75))
		var rooms := {3: [-1.0, "LAB"], 5: [1.0, "AQUARIUM"],
			9: [1.0, "MAP ROOM"], 11: [-1.0, "CARGO BAY"]}
		for ws in [1.0, -1.0]:
			if rooms.has(i) and float((rooms[i] as Array)[0]) == ws:
				# doorway to the side room: flanks + header
				_plate(Vector3(0.5, 6.0, 1.3), wxf, Vector3(ws * 5.25, 0, 1.85), STEEL, 0.0)
				_plate(Vector3(0.5, 6.0, 1.3), wxf, Vector3(ws * 5.25, 0, -1.85), STEEL, 0.0)
				_plate(Vector3(0.5, 1.8, 2.4), wxf, Vector3(ws * 5.25, 2.1, 0), STEEL, 0.0)
				_sign("%s %s" % ["<<" if ws < 0.0 else "", str((rooms[i] as Array)[1])]
					+ (" >>" if ws > 0.0 else ""), fb, _C + up * (_rF + 4.3),
					Vector3(ws * 4.6, 0, 0), 90.0 if ws < 0.0 else -90.0)
			else:
				_plate(Vector3(0.5, 6.0, 5.0), wxf, Vector3(ws * 5.25, 0, 0), STEEL, 0.0)
		if rooms.has(i):
			_side_room(i, float((rooms[i] as Array)[0]), str((rooms[i] as Array)[1]))
		# steel RIB: two studs + lintel, every segment
		_deco_box(Vector3(0.24, 6.0, 0.24), wxf, Vector3(5.0, 0, 2.3), STEEL, 0.0)
		_deco_box(Vector3(0.24, 6.0, 0.24), wxf, Vector3(-5.0, 0, 2.3), STEEL, 0.0)
		_deco_box(Vector3(10.24, 0.24, 0.24), wxf, Vector3(0, 2.9, 2.3), STEEL, 0.0)
		# conduit strips, both walls, continuous
		_deco_box(Vector3(0.06, 0.16, 5.0), wxf, Vector3(4.9, -1.6, 0), AMBER, 1.6)
		_deco_box(Vector3(0.06, 0.16, 5.0), wxf, Vector3(-4.9, -1.6, 0), AMBER, 1.6)
		if i % 2 == 0:
	...
	# ---- core room ----
	var ac := aend + (2.5 + 6.75) / _rF
	var cb := _fr(ac)
	var cup := _pdir(ac)
	_plate(Vector3(13.6, 0.5, 13.6),
		Transform3D(cb, _C + cup * (_rF - 0.25)), Vector3.ZERO, DARK, 0.0)
	_plate(Vector3(13.6, 0.5, 13.6),
		Transform3D(cb, _C + cup * (_rF + 7.25)), Vector3.ZERO, DARK, 0.0)
	var cwxf := Transform3D(cb, _C + cup * (_rF + 3.5))
	_plate(Vector3(1.8, 7.5, 0.5), cwxf, Vector3(5.9, 0, -6.55), STEEL, 0.0)
	_plate(Vector3(1.8, 7.5, 0.5), cwxf, Vector3(-5.9, 0, -6.55), STEEL, 0.0)
	_plate(Vector3(13.6, 7.5, 0.5), cwxf, Vector3(0, 0, 6.55), STEEL, 0.0)
	_plate(Vector3(0.5, 7.5, 13.6), cwxf, Vector3(6.55, 0, 0), STEEL, 0.0)
	_plate(Vector3(0.5, 7.5, 13.6), cwxf, Vector3(-6.55, 0, 0), STEEL, 0.0)
	# the reactor: pulsing cylinder + spinning rings + a guard collider
	var core := MeshInstance3D.new()
	var com := CylinderMesh.new()
	com.top_radius = 1.5
	com.bottom_radius = 1.7
	com.height = 4.6
	core.mesh = com
	_core_mat = StandardMaterial3D.new()
	_core_mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	_core_mat.albedo_color = AMBER.darkened(0.35)
	_core_mat.emission_enabled = true
	_core_mat.emission = AMBER
	core.material_override = _core_mat
	add_child(core)
	core.global_transform = Transform3D(cb, _C + cup * (_rF + 2.3))
	...
	cgb.add_child(cgc)
	add_child(cgb)
	cgb.global_transform = Transform3D(cb, _C + cup * (_rF + 2.3))
	for ri in 2:
		var ring := MeshInstance3D.new()
		var rgm := TorusMesh.new()
		rgm.inner_radius = 2.15
		rgm.outer_radius = 2.4
		ring.mesh = rgm
		ring.material_override = Destructible.make_material(
			AMBER.lightened(0.2), 1.8)
		add_child(ring)
		ring.global_transform = Transform3D(cb, _C + cup * (_rF + 1.6 + 1.5 * float(ri)))
		_core_rings.append({"node": ring, "spin": 0.5 + 0.4 * float(ri),
			"tilt": 0.18 + 0.14 * float(ri)})
	var fring := MeshInstance3D.new()
	var frm := TorusMesh.new()
	frm.inner_radius = 2.7
	frm.outer_radius = 2.95
	fring.mesh = frm
	fring.material_override = Destructible.make_material(AMBER, 1.3)
	add_child(fring)
	fring.global_transform = Transform3D(cb, _C + cup * (_rF + 0.08))
	# status wall: three framed panels
	for pi9 in 3:
		# framed status panel flat on the +Z wall: chamfered amber frame
		# behind a dark green readout pane
		var pfr := MeshInstance3D.new()
		pfr.mesh = IcosaColony._cham_mesh(3.4, 0.06, 2.2, 0.3)
		pfr.material_override = Destructible.make_material(AMBER, 1.3)
		add_child(pfr)
		pfr.global_transform = Transform3D(cb, _C + cup * (_rF + 4.0))
		pfr.rotate_object_local(Vector3(1, 0, 0), PI * 0.5)
		pfr.translate_object_local(Vector3(-4.2 + 4.2 * float(pi9), -6.2, 0))
		var pin := MeshInstance3D.new()
		var pim := BoxMesh.new()
		pim.size = Vector3(3.1, 1.9, 0.05)
		pin.mesh = pim
		pin.material_override = Destructible.make_material(Color("#0a2016"), 1.1)
		add_child(pin)
		pin.global_transform = Transform3D(cb, _C + cup * (_rF + 4.0))
		pin.translate_object_local(Vector3(-4.2 + 4.2 * float(pi9), 0, 6.15))
	_sign("CORE // DO NOT LICK", cb, _C + cup * (_rF + 5.6),
		Vector3(0, 0, 6.2), 180.0)
	_sign("<< CONTROL DECK / ATRIUM", cb, _C + cup * (_rF + 4.6),
		Vector3(0, 0, -6.2), 0.0)
...
func _room_lab(fb: Basis, up: Vector3, cx: float) -> void:
	...
	var ttm := CylinderMesh.new()
	ttm.top_radius = 0.0
	ttm.bottom_radius = 0.4
	ttm.height = 0.55
	ttm.radial_segments = 3
	tet.mesh = ttm
...
func _room_aquarium(fb: Basis, up: Vector3, cx: float, s: float) -> void:
	...
	for fi in 5:
		...
		fcm.height = 0.44
		fish.mesh = fcm
		fish.material_override = Destructible.make_material(
			[Color("#ffcf40"), Color("#ff6a6a"), Color("#7df9ff"),
			Color("#66ff99"), Color("#ff66aa")][fi], 1.2)
		add_child(fish)
		_fish.append({"node": fish, "fb": fb, "up": up, "x": s * 12.5,
			"phase": float(fi) * 1.7, "zr": 3.6, "yb": 1.0 + 0.5 * float(fi % 3)})
	_sign("AQUARIUM // FEEDING: AUTOMATED", fb, _C + up * (_rF + 3.7),
		Vector3(cx, 0, 4.2), 180.0)

func _room_map(fb: Basis, up: Vector3, cx: float) -> void:
	# the Dude system in miniature, orbiting over a pedestal
	var ped := MeshInstance3D.new()
	var pdm := CylinderMesh.new()
	pdm.top_radius = 1.3
	...
	ring.translate_object_local(Vector3(cx, 0, 0))
	var specs: Array = [[0.34, Color("#3a1d6e"), 1.0, 0.5],
		[0.3, Color("#0e3b2e"), 1.5, 0.34], [0.22, AMBER, 2.0, 0.26],
		[0.26, Color("#5a2a00"), 2.5, 0.2]]
	for gi in specs.size():
		var g := MeshInstance3D.new()
		var gm9 := SphereMesh.new()
		gm9.radius = float(specs[gi][0])
		gm9.height = gm9.radius * 2.0
		g.mesh = gm9
		g.material_override = Destructible.make_material(specs[gi][1], 1.3)
		add_child(g)
		_globes.append({"node": g, "fb": fb, "up": up, "cx": cx,
			"orbit": float(specs[gi][2]), "rate": float(specs[gi][3]),
			"phase": float(gi) * 1.9})
...
func _process(delta: float) -> void:
	...
	for sp in _spins:
		(sp["node"] as Node3D).rotate_object_local(Vector3(0, 1, 0),
			delta * float(sp["rate"]))
	# fish cruise their tank, globes orbit their pedestal
	for f in _fish:
		...
		fn.global_transform = Transform3D(
			(f["fb"] as Basis) * Basis(Vector3(1, 0, 0), PI * 0.5)
			* Basis(Vector3(0, 0, 1), (PI * 0.5 if cos(ph) > 0.0 else -PI * 0.5)),
			Transform3D(f["fb"] as Basis,
			_C + (f["up"] as Vector3) * (_rF + float(f["yb"])
			+ 0.25 * sin(ph * 1.4))).translated_local(Vector3(fx, 0, fz)).origin)
	for g in _globes:
		var gn: Node3D = g["node"]
		var ga := _t * float(g["rate"]) + float(g["phase"])
		gn.global_transform = Transform3D(g["fb"] as Basis,
			Transform3D(g["fb"] as Basis,
			_C + (g["up"] as Vector3) * (_rF + 1.7 + 0.3 * sin(ga * 0.7)))
			.translated_local(Vector3(float(g["cx"]) + cos(ga) * float(g["orbit"]),
			0, sin(ga) * float(g["orbit"]))).origin)
